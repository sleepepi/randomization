  ...
  def index
    ...
    respond_to do |format|
      ...
      format.json do # TODO: Put into jbuilder instead!
        render json: params[:search].to_s.split(',').collect{ |u| (u.strip.downcase == 'me') ? { name: current_user.name, id: current_user.name } : { name: u.strip.titleize, id: u.strip.titleize } } + @users.collect{ |u| { name: u.name, id: u.name } }
      end
    end
  end
